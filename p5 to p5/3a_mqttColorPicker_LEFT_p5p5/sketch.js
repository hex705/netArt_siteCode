...
// MQTT - shiftr 
// PUBLISH TOPIC
let myPublishTopic = "leftRGB";
let payload = "empty";

//SUBscribe topic
let mySubscribeTopic = "rightRGB"; 
...
function drawScreen(){
  ...
  // draw left color choice
  leftColor = color(rValue, gValue,bValue);
  fill(leftColor);
  rect(sideSpacer, topSpacer*2, base, base);
  
  // slider labels for left color
  textSize(17);
  fill(0);
  ...
  text('Blue  ('+bSlider.value()+')',350,290-topSpacer*3);
   ...
   // draw right color (remote to left)
   rightColor = color(receivedRed, receivedGreen, receivedBlue);
   fill(rightColor);
   ...
   //feedback message
   text('received from right ',sideSpacer,topSpacer*3+base);
   fill(87,87,87);
   textSize(16);
   text('color from right:: '+incomingPayload,350, 475);
}
...
function buildColorMessage(){
  ...
  let r = floor(red(leftColor));
  let g = floor(green(leftColor));
  let b = floor(blue(leftColor));
